...
chans = c("Laminin","Myh7","Myh2","Myh1","Area")
...
#Density distribution of fibre area to decide cut off for above
#Areagraph <- df %>%
# select(ID, Filename, Area)

#ggplot(Areagraph, aes(x=Area, color="blue")) + 
#geom_density(size=1)
#areacut off 11000

# Find the max value between numeric string
df2 <- df %>%
  select("Myh7", "Myh2", "Myh1")
...
data$FibreType <- ifelse(data$colMax == data$Myh2, 'Myh2+',
                         ifelse(data$colMax == data$Myh1, 'Myh1+', 'Myh7+'))
...
#Myh7 positive fibres
Myh7df <- data %>%
  select(Myh7, Area, Sample, Exercise, FibreType) %>%
  filter(FibreType=="Myh7+")

Myh7 <- ggplot(Myh7df, aes(x=Sample, y=Area)) +
...
for (fn in sort(unique(Myh7df$Sample))){
  pre = Myh7df$Area[(Myh7df$Sample==fn)&(Myh7df$Exercise==" Pre")]
  post = Myh7df$Area[(Myh7df$Sample==fn)&(Myh7df$Exercise=="Post")]
  print(fn)
...
Myh2df <- data %>%
  select(Myh2, Area, Sample, Exercise, FibreType) %>%
  filter(FibreType=="Myh2+")

Myh2 <- ggplot(Myh2df, aes(x=Sample, y=Area)) +
...
for (fn in sort(unique(Myh2df$Sample))){
  pre = Myh2df$Area[(Myh2df$Sample==fn)&(Myh2df$Exercise==" Pre")]
  post = Myh2df$Area[(Myh2df$Sample==fn)&(Myh2df$Exercise=="Post")]
  print(fn)
  print(t.test(pre,post,alternative="less"))
}
...
Myh1df <- data %>%
  select(Myh1, Area, Sample, Exercise, FibreType) %>%
  filter(FibreType=="Myh1+")

Myh1 <- ggplot(Myh1df, aes(x=Sample, y=Area)) +
  geom_boxplot(aes(colour=Exercise), width=0.8, alpha=0.6, 
               notch=TRUE,  outlier.shape=(NA),
               fill=c("magenta4", "grey40","magenta4", "grey40","magenta4", "grey40","magenta4", "grey40","magenta4", "grey40")) +
  geom_jitter(aes(colour=Exercise), alpha=0.3, shape=16, position=position_jitterdodge()) +
  ggtitle("Fibre type Myh1+") + 
  xlab("") +
  labs(y=expression(paste("Cross sectional area (µm"^{2}, ")"))) +
  ylim(0, 12000) + 
...
for (fn in sort(unique(Myh1df$Sample))){
  pre = Myh1df$Area[(Myh1df$Sample==fn)&(Myh1df$Exercise==" Pre")]
  post = Myh1df$Area[(Myh1df$Sample==fn)&(Myh1df$Exercise=="Post")]
  print(fn)
  print(t.test(pre,post,alternative="less"))
}
...
ggplot(P1, aes(x=Exercise, y=Percent, col=FibreType)) +
  ...
  scale_color_manual(values=c("magenta4", "firebrick2", "seagreen")) +
  ggtitle("P1") +
...
#Older Controls from Murgia's 2017
fname = "Controls.csv"
...
Control <- ggplot(C1, aes(x=FibreType, y=Percent, fill=FibreType)) +
  geom_bar(stat="identity") +
  #scale_fill_grey() + theme_minimal() +
  scale_fill_manual(values = alpha(c("magenta4", "firebrick2", "seagreen"), .5)) +
  ggtitle("C1") + 
  xlab("") +
  ylim(0,70)+
...
Control <- ggplot(C2, aes(x=FibreType, y=Percent, fill=FibreType)) +
  geom_bar(stat="identity") +
  #scale_fill_grey() + theme_minimal() +
  scale_fill_manual(values = alpha(c("magenta4", "firebrick2", "seagreen"), .5)) +
  ggtitle("C2") + 
...
Control <- ggplot(C3, aes(x=FibreType, y=Percent, fill=FibreType)) +
  geom_bar(stat="identity") +
  #scale_fill_grey() + theme_minimal() +
  scale_fill_manual(values = alpha(c("magenta4", "firebrick2", "seagreen"), .5)) +
  ggtitle("C3") + 
...
Control <- ggplot(C4, aes(x=FibreType, y=Percent, fill=FibreType)) +
...
Control <- ggplot(data_frac3, aes(x=FibreType, y=Mean, fill=FibreType)) +
  ...
  theme()
Control
pieCTRL <- Control + coord_polar("y", start=0) +
  theme(axis.text.x=element_blank())
#geom_text(aes(y = Value/4 + c(0, cumsum(Value)[-length(Value)]), label = percent(Value/100)), size=5)
pieCTRL
...
Pre <- ggplot(data_frac4, aes(x=FibreType, y=Mean, fill=FibreType)) +
  geom_bar(stat="identity", position="dodge") +
  #scale_fill_grey() + theme_minimal() +
  scale_fill_manual(values = alpha(c("magenta4", "firebrick2", "seagreen"), .5)) +
...
preI <- prop.test(x = c(2169,680), n = c(4253, 2215))
preI #p-value < 2.2e-16
#ctrl pre
#0.5099929 0.3069977  
pre2a <- prop.test(x = c(1701,1222), n = c(4253, 2215))
pre2a #p-value < 2.2e-16
#ctrl pre 
#0.399953 0.551693
pre2x <- prop.test(x= c(383,313), n= c(4253, 2215))
pre2x #p-value = 3.609e-10
#ctrl pre 
#0.09005408 0.14130926

#Controls vs post-exercise
postI <- prop.test(x = c(2169,1423), n = c(4253, 4897))
postI #p-value < 2.2e-16
#ctrl post
#0.5099929 0.2906  
post2a <- prop.test(x = c(1701,2715), n = c(4253, 4897))
post2a #p-value < 2.2e-16
#ctrl post 
#0.399953 0.5544
post2x <- prop.test(x= c(383,759), n= c(4253, 4897))
post2x #p-value < 2.2e-16
#ctrl post 
#0.09005408 0.1549
